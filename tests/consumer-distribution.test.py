...
def git(root, *args):
    # Temporary repositories cannot outlive their context. Keep Git's normal
    # maintenance, but wait for its writers before inspection or cleanup.
    result = run(['git', '-c', 'maintenance.autoDetach=false',
                  '-c', 'gc.autoDetach=false', *args], root)
...
def change_head_after_prepare(path,value,mode=0o644):
    ...
    if path.parent.name=='transactions' and value.get('stage')=='PREPARED':
        subprocess.run(['git','-c','maintenance.autoDetach=false','-c','gc.autoDetach=false',
                        '-C',target,'commit','--allow-empty','-qm','concurrent change'],
                       check=True)
