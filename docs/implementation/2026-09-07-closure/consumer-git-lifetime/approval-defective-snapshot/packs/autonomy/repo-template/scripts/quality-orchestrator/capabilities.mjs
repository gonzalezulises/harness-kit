...
import { checkBudget, mechanicalBudget } from './budget.mjs';
...
export function capabilityBoundary(host,runtime,auth,journal,files,continuation){
  ...
  function describe(id,input,ctx){
    ...
    const before=manifest(root),outputs=Object.create(null);permitted(args.path);
    ...
    const expectedOutputs=id==='canonical-write.v1'?[args.path]:[args.sourcePath,args.path];must(canonical(Object.keys(outputs).sort())===canonical(expectedOutputs.sort()),'closed capability output set mismatch');
  ...
  return {
    ...
    executeCapability:(permit,lease)=>safe(()=>{
      const item=permits.get(permit);must(item,'foreign or forged permit');const owner=owned(lease),{plan,ctx,grant}=item;fresh(ctx);const checked=item.reusable?continuation.inspect(grant,ctx,plan):runtime.inspectApproval(grant);must(checked.status===(item.reusable?'VERIFIED_CONTINUATION':'VERIFIED_APPROVAL'),checked.reason,checked.status);must(owner.runId===plan.runId,'lease belongs to different run','BLOCKED_BY_OWNERSHIP');
      ...
      const budget=checkBudget(journal.read(ctx).state.budget,mechanicalBudget);if(budget)fail(budget.reason,budget.status);
      ...
      const reserved=journal.append(plan.headDigest,{kind:'reserve',operationKey:plan.args.operationKey,runId:plan.runId,units:1,inputDigest:item.subjectDigest,effectKind:'local-capability.v1',budgetKind:mechanicalBudget,...(item.reusable?{grantDigest:checked.grantDigest}:{})},ctx);must(reserved.status==='APPENDED',reserved.reason||'reservation failed',reserved.status);
