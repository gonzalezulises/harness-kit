...
export function reviewBoundary(host,runtime,auth,execution,journal,internal={}){
  ...
  return {
    ...
    executeReviewCatalog:(wire,ctx)=>executionAsync(async()=>{wire=freeze(structuredClone(wire));const validateStart=()=>{fresh();must(canonical(wire?.request)===canonical(catalogRequest(wire?.request?.authMode)),'catalog request changed');};validateStart();return execution.execute(wire,ctx,validateStart);}),
