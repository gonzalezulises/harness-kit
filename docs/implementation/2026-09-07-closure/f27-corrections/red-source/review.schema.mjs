...
// Infrastructure fields from a model are hints only, never durable identity.
export const productReviewV2Schema=z.strictObject({version:z.literal(2),verdict:z.enum(['PASS','FAIL']),findings:z.array(z.strictObject({id:z.unknown().optional(),durable_test_id:z.unknown().optional(),severity:z.enum(['Critical','High','Medium','Low']),path:reviewPath,line:positive,description:z.string().min(1).max(16000),counterexample:z.string().min(1).max(16000).nullable(),evidence_refs:z.array(z.string().max(1000)).min(1).max(100)})).max(100)});
export const productReviewV2JSONSchema=z.toJSONSchema(productReviewV2Schema);
export const productReviewV2SchemaDigest=digestData(productReviewV2JSONSchema);
