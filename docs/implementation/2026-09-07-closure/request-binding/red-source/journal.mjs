...
export function journalBoundary(host,runtime,auth,contextPaths,internal={}) {
  ...
  function append(expectedHead,op,ctx) {return appendOwned(expectedHead,op,ctx).receipt;}
  function appendOwned(expectedHead,op,ctx,validateNew) {
    return exclusive(()=>{
  ...
  Object.assign(internal,{read,append,finalBinding,putObject,getObject,
    claimExecution:(expectedHead,op,ctx,validateNew)=>{must(op.kind==='reserve'&&backendOwned(op)&&typeof validateNew==='function','private execution reservation required','POLICY');return appendOwned(expectedHead,op,ctx,validateNew);},
