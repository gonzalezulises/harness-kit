...
export function reviewManifest(root){
  const entries=Object.create(null);let count=0,size=0;
...
export function buildReviewShadow(config,commit){
  ...
  try {
    ...
    const objectRoot=path.join(repo,'.git/objects');let total=0,count=0;const copied=new Map(),files=Object.create(null);
    ...
    requireReview(primaryFiles.length===Object.keys(files).length&&primaryFiles.every(([p,v])=>files[p]?.digest===v.digest&&v.mode===(files[p].mode==='100755'?0o755:0o644)),'primary files differ from exact target commit');
