...
const start=z.strictObject({kind:z.literal('start'),operationKey:id,baselineDigest:hash,budgetLimit:integer,run:runSchema});
const replace=z.strictObject({kind:z.literal('replace-run'),operationKey:id,oldRunId:hash,run:runSchema});
const reserve=z.strictObject({kind:z.literal('reserve'),operationKey:id,runId:hash,units:integer.refine(v=>v>0),inputDigest:hash});
const close=z.strictObject({kind:z.literal('close-run'),operationKey:id,runId:hash,verdict:z.enum(['STALE','FAILED','INCOMPLETE'])});
const outcome=z.strictObject({kind:z.literal('outcome'),operationKey:id,intentKey:id,status:z.enum(['COMPLETED','NOT_APPLIED']),outputDigest:hash.nullable()}).refine(v=>v.status==='COMPLETED'?v.outputDigest!==null:v.outputDigest===null);
const legacy=z.strictObject({kind:z.literal('legacy-import'),operationKey:id,format:z.literal('feature-list.v1'),bytesBase64:base64,sourceDigest:hash});
...
export function journalBoundary(host,runtime,auth,contextPaths) {
  ...
  function validateTransition(before,op,ctx) {
    ...
    if(op.kind==='start'){must(state.sequence===0&&op.baselineDigest===state.baselineDigest&&op.budgetLimit===config.budgetLimit,'objective already started','POLICY');validateBinding(op.run.binding,ctx);}
      ...
      if(op.kind==='reserve'){must(current.runId===op.runId&&current.verdict==='OPEN','current open run required','POLICY');must(!state.pending.length,'unreconciled operation intent');must(op.units<=state.budget.limit-state.budget.spent,'objective budget exhausted','BUDGET_EXHAUSTED');}
  ...
  return Object.freeze({
    ...
    projectLegacy:ctx=>safe(()=>{const state=read(ctx).state;return freeze({mode:'v2-read-only',writable:false,provenance:{assurance:state.assurance,journalId:state.journalId,sequence:state.sequence,headDigest:state.headDigest,sourceDigest:state.legacy?.sourceDigest||null},features:state.legacy?legacyFeatures(Buffer.from(state.legacy.bytesBase64,'base64')).map(f=>({id:f.id,state:'LEGACY_UNVERIFIED'})):[],runs:state.runs});})
  });
}
