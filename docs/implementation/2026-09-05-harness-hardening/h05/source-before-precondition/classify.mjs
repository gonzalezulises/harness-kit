...
import { journalBoundary } from './journal.mjs';
...
export function openRuntime(host) {
  ...
  function verifyContext(request) {
    try {
      ...
      if(acceptance.status)return acceptance;
      const checked=auth.inspectApproval(acceptance);if(checked.status!=='VERIFIED_APPROVAL')return checked;
  ...
  const runtime={identify,describeBaseline,verifyContext,inspectContext,classifyChange,loadAuthority:auth.loadAuthority,inspectAuthority:auth.inspectAuthority,verifyApproval:auth.verifyApproval,inspectApproval:auth.inspectApproval};
  const journalInternal={};
  if(host.journal) {
    try {runtime.journal=journalBoundary(host.journal,runtime,auth,handle=>contexts.get(handle).baseline.records.map(r=>r.path),journalInternal);}
    catch(error){return stop('BLOCKED_BY_MISSING_AUTHORITY_BINDING',error.message);}
  }
  if(host.capabilities){try{if(!host.journal)throw Error('journal required for capabilities');Object.assign(runtime,capabilityBoundary(host,runtime,auth,journalInternal,files));}catch(error){return stop(error.capabilityStatus||'BLOCKED_BY_MISSING_AUTHORITY_BINDING',error.message);}}
