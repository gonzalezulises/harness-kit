...
import { Parser, parseDocument, isMap, isSeq, isScalar, isAlias } from 'yaml';
import { z } from 'zod';
...
const numberLexeme = z.string().regex(/^[+-]?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?$/);
class ExactNumeric { constructor(lexeme) { this.lexeme=lexeme; } }
const exactNumber=z.custom(value=>value instanceof ExactNumeric).transform(value=>({type:'number',lexeme:value.lexeme}));
const contracts = {
...
function typed(node) {
  if (!node || node.tag || node.anchor || node.comment !== undefined || node.commentBefore !== undefined || isAlias(node)) throw Error('unsupported YAML node');
  if (isMap(node)) {
    const result=Object.create(null);
    for (const pair of node.items) {
      if (!isScalar(pair.key) || typeof pair.key.value !== 'string' || pair.key.tag || pair.key.anchor || pair.key.comment !== undefined || pair.key.commentBefore !== undefined || pair.key.value==='<<' || Object.hasOwn(result,pair.key.value)) throw Error('unsupported mapping key');
  ...
  if (typeof node.value==='number' || typeof node.value==='bigint') {
    ...
    if (!numberLexeme.safeParse(lexeme).success) throw Error('unsupported numeric form');
    return new ExactNumeric(lexeme);
  }
...
export function identifyForRepository(repositoryId, bytes, schemaId) {
  try {
    ...
    // The composed tag map includes implicit defaults and loses the distinction
    // between an implicit !! handle and an explicit declaration of that handle.
    for (const token of new Parser().parse(source)) {
      if (token.type==='directive') throw Error('unsupported YAML directive');
    }
    ...
    if(document.comment !== undefined && document.comment !== null || document.commentBefore !== undefined && document.commentBefore !== null || document.errors.length || document.warnings.length || document.directives.yaml.explicit || document.directives.tags && Object.keys(document.directives.tags).some(key=>key!=='!!')) throw Error('invalid or unsupported YAML');
