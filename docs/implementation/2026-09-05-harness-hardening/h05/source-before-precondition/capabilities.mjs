...
export function capabilityBoundary(host,runtime,auth,journal,files){
  ...
  function describe(id,input,ctx){
    ...
    for(const [p,output] of Object.entries(outputs)){must(before[p]?.kind==='file','existing regular output required');must(Buffer.byteLength(output)<=1024*1024,'output bound exceeded');}
    const assessment=runtime.classifyChange({changes:Object.entries(outputs).map(([p,value])=>({path:p,after:Buffer.from(value)}))},ctx);must(assessment.disposition==='MECHANICAL_ELIGIBLE','outputs do not preserve accepted equivalence');
  ...
  function complete(plan,ctx,lease){
    ...
    const loaded=journal.read(ctx),intent=loaded.intents.get(plan.args.operationKey);must(intent&&intent.inputDigest===digestData(plan)&&intent.runId===plan.runId,'persistent intent differs from operation');
    ...
    const final=journal.finalBinding(ctx);must(final.commit===plan.binding.commit,'commit changed during effect','INCOMPLETE');
  ...
  return {
    ...
    executeCapability:(permit,lease)=>safe(()=>{
      ...
      const reserved=journal.append(plan.headDigest,{kind:'reserve',operationKey:plan.args.operationKey,runId:plan.runId,units:1,inputDigest:item.subjectDigest},ctx);must(reserved.status==='APPENDED',reserved.reason||'reservation failed',reserved.status);
