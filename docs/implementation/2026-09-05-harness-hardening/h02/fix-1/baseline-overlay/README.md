...
|---|---|
| `READY_DUAL` | Check local observado y reglas remotas inspeccionadas para check e integridad. No sustituye un canario adversarial. |
| `READY_PARTIAL` | Check local observado y regla remota requerida inspeccionada; falta integridad del workflow. |
| `READY_LOCAL` | `make check` produjo cero; no se observó una regla remota requerida. |
...
`bash` 3.2+, `git` y Python3. El scaffold minimal conserva esas dependencias; el nivel full requiere además PyYAML6.0.3 en su entorno aislado.
...
## Verificación viva y perfiles de instalación

La capacidad actual y sus límites están en [harness-capabilities.md](docs/harness-capabilities.md). `make check` aplica gates quick a la configuración viva antes de las suites. El perfil v1 distingue kit, consumidor full y scaffold minimal; sólo el kit exige sincronizar sus cuatro archivos de versión. La ausencia de un gate aplicable bloquea.

`./init.sh` / `make test` verifican sólo el núcleo; `make check` incluye los packs. El parser de oracles requiere PyYAML6.0.3, instalado de forma aislada con `bash scripts/setup-oracles.sh`. El full del kit necesita también Node, k6 y curl. Los dos casos de GitHub autenticado omitidos siguen siendo cobertura no ejecutada. El hook conserva el índice parcial y los bytes del workspace; un formatter fallido bloquea.

La adopción inicial en CI requiere un juez/perfil protegido compatible. Su ausencia produce ADOPTION_REQUIRED y exige la acción de mantenimiento del owner descrita en el documento; esta PR no acepta una baseline ni despliega productos.
