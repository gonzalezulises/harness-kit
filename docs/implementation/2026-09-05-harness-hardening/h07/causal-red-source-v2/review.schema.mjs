...
export const reviewPolicySchema=reviewInputSchema.extend({version:z.literal(1),repositoryId:id,scope:z.strictObject({objectiveId:id,operationKey:id,budgetKind:reviewInputSchema.shape.budgetKind,targetCommit:reviewInputSchema.shape.targetCommit}),pins:z.strictObject({adapterDigest:hash,artifactsDigest:hash,configDigest:hash,schemaDigest:hash,containment:z.literal('UNAVAILABLE')})});
export const reviewSchema=z.object({version:z.literal(1),verdict:z.enum(['PASS','FAIL']),findings:z.array(z.strictObject({id,severity:z.enum(['Critical','High','Medium','Low']),path:reviewPath,line:positive,description:z.string().min(1).max(16000),counterexample:z.string().min(1).max(16000).nullable()})).max(100)});
