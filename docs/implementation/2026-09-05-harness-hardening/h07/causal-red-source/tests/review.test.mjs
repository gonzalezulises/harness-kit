...
test('e2e case 7: exact shadow and signed objective attempt stop before unavailable reviewer launch',t=>{
  const s=setup(t),b=s.bind(),shadow=s.r.createReviewShadow(b),before=fs.readFileSync(path.join(s.primary,'record.yaml'));
  const result=s.r.reviewRun(b,shadow);assert.equal(result.status,'BLOCKED_BY_REQUIRED_CAPABILITY');assert.equal(result.execution,'NOT_EXECUTED');assert.equal(result.acceptance,'NOT_EXECUTED');assert.equal(result.rawOutput,null);assert.equal(result.executionReceipt,null);assert.equal(result.budgetReceipt.status,'APPENDED');
  assert.deepEqual(s.r.reviewRun(b,shadow),result);assert.equal(s.r.journal.replay(s.ctx).budget.byKind[harness],1);assert.deepEqual(fs.readFileSync(path.join(s.primary,'record.yaml')),before);
  const next=s.bind({operationKey:'second'}),nextShadow=s.r.createReviewShadow(next);assert.equal(s.r.reviewRun(next,nextShadow).status,'BUDGET_EXHAUSTED');
});
