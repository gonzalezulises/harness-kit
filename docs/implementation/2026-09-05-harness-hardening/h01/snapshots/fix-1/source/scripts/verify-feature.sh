...
for i, feature in enumerate(features):
    ...
    if not isinstance(feature_id, str) or not feature_id.strip():
        reject(where + ".id must be a non-empty string")
    ...
    for j, layer in enumerate(layers):
        ...
        if not isinstance(label, str) or not label.strip():
            reject(layer_where + ".label must be a non-empty string")
        if not isinstance(command, str) or not command.strip():
            reject(layer_where + ".cmd must be a non-empty string")
        if not isinstance(repair, str):
            reject(layer_where + ".repair must be a string")
    ...
    if budgets is not None:
        ...
        if not isinstance(budgets.get("stop_condition"), str) or not budgets["stop_condition"].strip():
            reject(where + ".budgets.stop_condition must be a non-empty string", 67)
    ...
    if ledger is not None:
        if not isinstance(ledger, dict):
            reject(where + ".ledger must be an object")
        ...
        for j, blocker in enumerate(blockers):
            ...
            if not isinstance(signature, str) or not signature.strip():
                reject(f"{where}.ledger.blockers[{j}].signature must be a non-empty string")
