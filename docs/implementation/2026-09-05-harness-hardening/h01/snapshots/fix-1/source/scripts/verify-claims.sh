...
for i, feature in enumerate(features):
    ...
    fid = feature.get("id")
    if not isinstance(fid, str) or not fid.strip(): reject(where + ".id must be a non-empty string")
    ...
    for j, layer in enumerate(layers):
        ...
        if not isinstance(layer.get("label", "layer"), str) or not layer.get("label", "layer").strip():
            reject(lw + ".label must be a non-empty string")
        if not isinstance(layer.get("cmd"), str) or not layer["cmd"].strip():
            reject(lw + ".cmd must be a non-empty string")
        if not isinstance(layer.get("repair", "No repair guidance recorded."), str):
            reject(lw + ".repair must be a string")
    budgets = feature.get("budgets")
    if budgets is not None:
        if not isinstance(budgets, dict): reject(where + ".budgets must be an object")
        if not isinstance(budgets.get("stop_condition"), str) or not budgets["stop_condition"].strip():
            reject(where + ".budgets.stop_condition must be a non-empty string")
        for key in ("review_rounds_max", "repeated_blocker_max"):
            if key in budgets:
                value = budgets[key]
                if isinstance(value, bool) or not isinstance(value, int) or value < 0:
                    reject(f"{where}.budgets.{key} must be a non-negative integer")
    ledger = feature.get("ledger")
    if ledger is not None:
        if not isinstance(ledger, dict): reject(where + ".ledger must be an object")
        rounds, blockers = ledger.get("review_rounds", 0), ledger.get("blockers", [])
        ...
        for j, blocker in enumerate(blockers):
            ...
            if not isinstance(signature, str) or not signature.strip(): reject(bw + ".signature must be non-empty")
    ...
    for i, feature in enumerate(data["features"]):
        ...
        if ledger is not None:
            ...
            for blocker in blockers:
                ...
                signature, count = blocker.get("signature"), blocker.get("count")
                if (not isinstance(signature, str) or not signature.strip() or signature in signatures
