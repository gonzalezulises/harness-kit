...
for(const where of ['primary','shadow'])test(`prototype-shaped filename remains covered by ${where} manifest`,t=>{
  const s=setup(t);fs.writeFileSync(path.join(s.primary,'__proto__'),'frozen target\n');s.g('add','__proto__');s.g('commit','-m','prototype-shaped file');
  const b=s.bind({targetCommit:s.g('rev-parse','HEAD')}),shadow=s.r.createReviewShadow(b),info=s.r.inspectReviewShadow(shadow);assert.equal(info.status,'REVIEW_SHADOW_BUILT',info.reason);
  fs.appendFileSync(path.join(where==='primary'?s.primary:info.directory,'__proto__'),'tampered\n');assert.equal(s.r.inspectReviewShadow(shadow).status,'POLICY');
});
