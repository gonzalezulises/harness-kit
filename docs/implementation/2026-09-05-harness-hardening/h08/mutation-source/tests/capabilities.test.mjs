...
function setup(t,options={}) {
  ...
  const f=fixture(options.authorityPaths?{authorityOverrides:{scope:{paths:options.authorityPaths}}}:{});const documents=(options.acceptedPaths||(options.onlySource?['record.yaml','policy.yaml']:['record.yaml','digest.yaml','policy.yaml'])).map(p=>({path:p,bytes:fs.readFileSync(path.join(root,p))}));
...
for(const scenario of [
  {name:'canonical output missing accepted-before identity',id:'canonical-write.v1',acceptedPaths:['policy.yaml']},
  {name:'canonical output outside adopted scope',id:'canonical-write.v1',acceptedPaths:['policy.yaml'],authorityPaths:['policy.yaml']},
  {name:'derived target omitted from accepted context',id:'derived-write.v1',acceptedPaths:['record.yaml','policy.yaml']},
  {name:'derived source and target outside adopted context',id:'derived-write.v1',acceptedPaths:['policy.yaml'],authorityPaths:['policy.yaml']}
])test(`H05-R1 rejects ${scenario.name} before permit, spend or effects`,t=>{
  const s=setup(t,scenario),args=scenario.id==='canonical-write.v1'?{path:'record.yaml',operationKey:'unaccepted'}:{...s.args,operationKey:'unaccepted'};
  const outputPaths=scenario.id==='canonical-write.v1'?['record.yaml']:['record.yaml','digest.yaml'];
  const before=Object.fromEntries(outputPaths.map(p=>[p,fs.readFileSync(path.join(s.root,p))]));
  const omitted=outputPaths.find(p=>!scenario.acceptedPaths.includes(p));
  assert.equal(s.r.classifyChange({changes:[{path:omitted,after:before[omitted]}]},s.ctx).disposition,'UNKNOWN');
  const description=s.r.describeCapability(scenario.id,args,s.ctx);let observed=description;
  // On the defective implementation, exercise the real exact signed grant and
  // lease route so RED observes false verified effects, not only a description.
  if(description.status==='CAPABILITY_DESCRIBED'){
    const permit=s.r.prepareCapability(scenario.id,args,s.ctx,s.f.receipt('bounded-grant',description.subjectDigest,{scopeDigest:description.scopeDigest}));
    const lease=s.r.acquireLease(s.ctx);observed=s.r.executeCapability(permit,lease);s.r.releaseLease(lease);
  }
  assert.equal(observed.status,'POLICY');
  assert.equal(s.r.prepareCapability(scenario.id,args,s.ctx,{}).status,'POLICY');
  for(const p of outputPaths)assert.deepEqual(fs.readFileSync(path.join(s.root,p)),before[p]);
  const state=s.r.journal.replay(s.ctx);assert.equal(state.budget.spent,0);assert.deepEqual(state.pending,[]);
  assert.equal(fs.readdirSync(path.join(s.dir,'state/capabilities')).some(n=>n.endsWith('.intent')||n.endsWith('.receipt')),false);
});
