...
test('human comments and directives are unsupported instead of silently erased',()=>{
 for(const text of [sample+'# owner: threshold must never decrease\n',sample.replace('100','100 # critical'),'%YAML 1.2\n---\n'+sample])assert.equal(id(text).status,'UNKNOWN');
});
test('a YAML mapping cannot impersonate the internal numeric domain representation',()=>{
 const mapped=sample.replace('threshold: 100','threshold:\n  type: number\n  lexeme: "100"');
 assert.equal(id(mapped).status,'UNKNOWN');
});
test('explicit default-handle TAG directives reject instead of inheriting implicit defaults',()=>{
 for(const directive of ['%TAG !! tag:yaml.org,2002:\n','%TAG !! tag:yaml.org,2002:\r\n','%TAG !e! tag:example.test,2026:\n']) {
  assert.equal(id(directive+'---\n'+sample).status,'UNKNOWN');
 }
 const literal=id(sample.replace('title: hello','title: "%TAG !! tag:yaml.org,2002:"'));
 assert.equal(literal.status,'IDENTIFIED');
 assert.equal(literal.value.title,'%TAG !! tag:yaml.org,2002:');
});
