...
export const kinds=Object.freeze(['policy-adoption','baseline-acceptance','bounded-grant','deployment-authorization','recovery','journal-checkpoint']);
...
export function authorityBoundary(host) {
  ...
  function verify(input,expected,recordedAt) {
    try {
  ...
  const verifyApproval=(input,expected)=>verify(input,expected);
  const verifyRecordedApproval=(input,expected,at)=>verify(input,expected,at);
