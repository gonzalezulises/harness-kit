...
import { budgetLimitsSchema, budgetKindSchema, checkBudget, mechanicalBudget } from './budget.mjs';
...
export function continuationBoundary(runtime,auth,journal,files,acceptedRecords,internal) {
  ...
  function verify(wire,ctx,observed) {
    ...
    const binding=journal.finalBinding(ctx);must(canonical(binding)===canonical(state.runs.at(-1)?.binding),'fresh current run required','BLOCKED_BY_STALE_RUN');
  ...
  function inspect(handle,ctx,plan) {
    const item=handles.get(handle);must(item&&item.ctx===ctx,'foreign or forged continuation');const checked=verify(item.wire,ctx);
    ...
    const budget=checkBudget(checked.state.budget,mechanicalBudget);if(budget)fail(budget.reason,budget.status);return checked;
  }
  Object.assign(internal,{has:handle=>handles.has(handle),inspect:(handle,ctx,plan)=>safe(()=>{inspect(handle,ctx,plan);return {status:'VERIFIED_CONTINUATION'};})});
  return {
