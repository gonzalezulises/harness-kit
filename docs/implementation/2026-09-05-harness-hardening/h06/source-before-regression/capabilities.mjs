...
export function capabilityBoundary(host,runtime,auth,journal,files,continuation){
  ...
  return {
    ...
    executeCapability:(permit,lease)=>safe(()=>{
      ...
      const reserved=journal.append(plan.headDigest,{kind:'reserve',operationKey:plan.args.operationKey,runId:plan.runId,units:1,inputDigest:item.subjectDigest,effectKind:'local-capability.v1',budgetKind:mechanicalBudget},ctx);must(reserved.status==='APPENDED',reserved.reason||'reservation failed',reserved.status);
