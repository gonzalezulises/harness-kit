...
for(const operation of ['loadAuthority','verifyContext']) {
 for(const failure of ['expired','unavailable']) test(operation+' preserves the '+failure+' stop at constructor reinspection',()=>{
  let readings=null;const observed=[];
  const f=fixture({now:()=>{
   if(readings===null)return 1000;
   const value=readings.shift();observed.push(value);
   if(value==='unavailable')throw Error('trusted clock disconnected');
   return value;
  }});
  const finalReading=failure==='expired'?1600:'unavailable';
  const sequence=operation==='loadAuthority'?[1000,finalReading]:[1000,1000,finalReading];
  readings=[...sequence];
  const result=operation==='loadAuthority'
   ?f.runtime.loadAuthority(bytes(canonical(f.authority)),f.receipt('policy-adoption',f.authorityDigest))
   :f.context();
  assert.deepEqual(result,{status:'BLOCKED_BY_STALE_AUTHORITY',reason:failure==='expired'?'approval is not current':'trusted time source unavailable'});
  assert.deepEqual(observed,sequence);
  assert.equal(f.runtime.inspectAuthority(result).status,'UNKNOWN');
  assert.equal(f.runtime.inspectContext(result).status,'UNKNOWN');
 });
 test(operation+' preserves an original verifyApproval signature stop',()=>{
  const f=fixture();
  const original=operation==='loadAuthority'
   ?f.receipt('policy-adoption',f.authorityDigest)
   :f.receipt('baseline-acceptance',f.runtime.describeBaseline([{path:'record.yaml',bytes:bytes('title: hello\nstatus: active\nenabled: true\nthreshold: 100\n')}]).digest);
  const invalid={...original,signature:Buffer.alloc(64).toString('base64')};
  const result=operation==='loadAuthority'
   ?f.runtime.loadAuthority(bytes(canonical(f.authority)),invalid)
   :f.runtime.verifyContext({authority:f.adopted,documents:[{path:'record.yaml',bytes:bytes('title: hello\nstatus: active\nenabled: true\nthreshold: 100\n')}],acceptance:invalid});
  assert.deepEqual(result,{status:'BLOCKED_BY_INVALID_SIGNATURE',reason:'signature verification failed'});
 });
}
