...
import { openRuntime } from '../index.mjs';
...
for(const [name,changes] of Object.entries({repo:{repositoryId:'other'},subject:{subjectDigest:'0'.repeat(64)},scope:{scopeDigest:'0'.repeat(64)},kind:{kind:'bounded-grant'},role:{issuerRole:'bot'},issuer:{issuer:'attacker'},key:{keyId:'two'},expired:{expiresAt:999},future:{issuedAt:1001},epoch:{revocationEpoch:6},decision:{decision:'reject'}})) test('receipt '+name+' blocks even when signed',()=>{
 const f=fixture();const got=f.runtime.loadAuthority(bytes(JSON.stringify(f.authority)),f.receipt('policy-adoption',f.authorityDigest,changes));assert.notEqual(got.status,undefined);
...
test('altered receipt signature and unknown fields block',()=>{
 ...
 for(const altered of [{...r,scopeDigest:'0'.repeat(64)},{...r,issuerRole:'admin'},{...r,trust:true},{...r,signature:'AAAA'}]) assert.equal(f.runtime.inspectAuthority(f.runtime.loadAuthority(bytes(JSON.stringify(f.authority)),altered)).status,'UNKNOWN');
});
