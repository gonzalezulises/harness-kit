...
export const kinds=Object.freeze(['policy-adoption','baseline-acceptance','bounded-grant','deployment-authorization','recovery']);
...
export const authoritySchema=z.strictObject({version:z.literal(1),repositoryId:nonempty,policyId:z.literal('conservative.v1'),role:z.literal('normative'),scope:z.strictObject({paths:z.array(nonempty).min(1)})});
...
export function authorityBoundary(host) {
  ...
  function loadAuthority(bytes,receipt) {
    try {
      ...
      if(inspectApproval(approval).status!=='VERIFIED_APPROVAL') return approval;
