...
time.sleep(float(os.environ.get("HARNESS_TEST_SERVER_DELAY", "0")))
...
SERVER_PIDS=""
start_server() {
  python3 "$WORK/server.py" "$1" "$WORK/$1.port" >"$WORK/$1.server.log" 2>&1 &
  local pid=$!
  SERVER_PIDS="$SERVER_PIDS $pid"
  if ! python3 - "$WORK/$1.port" "$pid" "${HARNESS_TEST_READY_TIMEOUT:-10}" <<'READY'
import os, pathlib, socket, sys, time
path, pid, timeout = pathlib.Path(sys.argv[1]), int(sys.argv[2]), float(sys.argv[3])
end = time.monotonic() + timeout
while time.monotonic() < end:
    try:
        os.kill(pid, 0)
        if path.is_file():
            with socket.create_connection(('127.0.0.1', int(path.read_text())), timeout=.2):
                sys.exit(0)
    except ProcessLookupError:
        print('TOOL_FAILURE: owned fixture server exited before readiness', file=sys.stderr); sys.exit(3)
    except (OSError,ValueError): pass
    time.sleep(.02)
print('INCOMPLETE: bounded fixture readiness timeout', file=sys.stderr)
sys.exit(4)
READY
  then
    say "verify-pack: server $1 did not reach readiness within the bound"
    cat "$WORK/$1.server.log"
    exit 3
  fi
  printf -v "$2" '%s' "$(cat "$WORK/$1.port")"
  say "fixture-server $1 owned-pid=$pid port=$(cat "$WORK/$1.port") work=$WORK"
}
start_server healthy HEALTHY_PORT
start_server redirect-login REDIRECT_PORT
start_server login-200 LOGIN_PORT
start_server slow SLOW_PORT
start_server unauthorized UNAUTHORIZED_PORT
start_server shell SHELL_PORT
...
expect "app sana con marcador aprueba" zero "APROBADO" \
  PERF_TARGET=http://127.0.0.1:${HEALTHY_PORT} PERF_APP_MARKER=$MARKER PERF_P95_MS=2000
...
expect "redirección a pantalla de autenticación" nonzero "pantalla de autenticación" \
  PERF_TARGET=http://127.0.0.1:${REDIRECT_PORT} PERF_AUTH=iap PERF_IAP_TOKEN=x PERF_APP_MARKER=$MARKER
expect "200 sin el marcador de la aplicación" nonzero "PERF_APP_MARKER" \
  PERF_TARGET=http://127.0.0.1:${LOGIN_PORT} PERF_AUTH=iap PERF_IAP_TOKEN=x PERF_APP_MARKER=$MARKER
expect "objetivo autenticado sin marcador declarado" nonzero "exige PERF_APP_MARKER" \
  PERF_TARGET=http://127.0.0.1:${LOGIN_PORT} PERF_AUTH=iap PERF_IAP_TOKEN=x
expect "credencial rechazada con 401" nonzero "rechazó la credencial" \
  PERF_TARGET=http://127.0.0.1:${UNAUTHORIZED_PORT} PERF_AUTH=bearer PERF_BEARER=x PERF_APP_MARKER=$MARKER
expect "credencial ausente para el modo declarado" nonzero "PERF_IAP_TOKEN" \
  PERF_TARGET=http://127.0.0.1:${HEALTHY_PORT} PERF_AUTH=iap PERF_APP_MARKER=$MARKER
...
expect "umbral de latencia incumplido" nonzero "umbral" \
  PERF_TARGET=http://127.0.0.1:${SLOW_PORT} PERF_APP_MARKER=$MARKER PERF_P95_MS=50

# Línea base tomada del servidor rápido, medida contra el lento: con el umbral
# holgado, sólo la deriva puede bloquear.
( cd "$WORK/fixture" && env PERF_TARGET=http://127.0.0.1:${HEALTHY_PORT} \
...
expect "degradación frente a la línea base" nonzero "se degradó" \
  PERF_TARGET=http://127.0.0.1:${SLOW_PORT} PERF_APP_MARKER=$MARKER PERF_P95_MS=2000
...
bash -c "cd '$WORK/fixture' && PERF_TARGET=http://127.0.0.1:${HEALTHY_PORT} PERF_AUTH=supabase \
...
bash "${TEMPLATE}/bin/perf-discover" http://127.0.0.1:${HEALTHY_PORT} "/" >"$DISC" 2>&1
...
bash "${TEMPLATE}/bin/perf-discover" http://127.0.0.1:${SHELL_PORT} "/" >"$WORK/disc-shell.log" 2>&1
...
bash "${TEMPLATE}/bin/perf-discover" http://127.0.0.1:${REDIRECT_PORT} "/" >"$WORK/disc-redir.log" 2>&1
...
if printf '%s' "$MARKER_LINE" | grep -q "PERF_APP_MARKER='"; then
  ...
  curl -s --max-time 8 http://127.0.0.1:${HEALTHY_PORT}/ > "$WORK/probe-body.html"
