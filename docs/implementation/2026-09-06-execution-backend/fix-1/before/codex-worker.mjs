...
export async function runCodexWorker(host,request){
  ...
  try{
    ...
    let sequence=0,buffer='',total=0,fatal=null,turnWait=null,turnNotifications=[],finishing=false;const pending=new Map();
    ...
    child.stdout.on('data',chunk=>{try{
      total+=chunk.length;must(total<=4*1024*1024,'worker output bound exceeded');buffer+=chunk.toString('utf8');must(Buffer.byteLength(buffer)<=2*1024*1024,'worker protocol frame bound exceeded');
