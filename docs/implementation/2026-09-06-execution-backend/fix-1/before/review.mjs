...
export function reviewBoundary(host,runtime,auth,execution,journal,internal={}){
  ...
  return {
    ...
    reviewRun:(binding,shadow,wire)=>{if(execution?.profile.review)return executionAsync(async()=>{const item=owned(binding);must(canonical(wire?.request)===canonical(reviewRequest(binding,shadow)),'review request changed');must(wire.budget.scope.operationKey===item.policy.operationKey&&wire.budget.scope.budgetKind===item.policy.budgetKind,'review budget scope mismatch');return execution.execute(wire,item.ctx);});return safe(()=>{
