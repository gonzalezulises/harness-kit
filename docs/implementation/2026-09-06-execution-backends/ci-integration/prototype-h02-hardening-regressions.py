...
import hashlib, json, os, pathlib, shutil, subprocess, tempfile, time, unittest
...
def workflow_run_step(name):
    """Return one literal run block from the shipped required-quality workflow."""
...
class Regressions(unittest.TestCase):
    ...
    def test_protected_workflow_uses_relocated_base_bundle(self):
        workspace = self.root/'workspace'
        base, target = workspace/'base', workspace/'head'
        judge = base/'.harness/protected-judge/v1'
        for repo in [base, target]:
            repo.mkdir(parents=True)
            command(repo, 'git', 'init', '-q', '-b', 'main')
            command(repo, 'git', 'config', 'user.email', 'fixture@example.test')
            command(repo, 'git', 'config', 'user.name', 'H02 fixture')
        for repo in [judge, target]:
            repo.mkdir(parents=True, exist_ok=True)
            p = command(base if repo == judge else target, 'bash', str(KIT/'bin/harness-init.sh'), '--target', str(repo), '--level', 'full')
            self.assertEqual(p.returncode, 0, p.stdout+p.stderr)
        ...
        for repo in [base, target]:
            ...
            p = command(repo, 'git', 'commit', '-qm', 'fixture')
            self.assertEqual(p.returncode, 0, p.stdout+p.stderr)

        # Setup's isolation is covered separately. This wrapper avoids a network
        # install while the workflow executes its real contract and quick gates.
        mock = workspace/'mock'; mock.mkdir()
        ...
        python.chmod(0o755)
        runner = workspace/'runner'; runner.mkdir()
        github_path = workspace/'github-path'
        env = {
            'GITHUB_WORKSPACE': str(workspace),
            'PROTECTED_JUDGE_ROOT': str(judge),
        ...
        script = workflow_run_step('Apply protected live policy to exact head')
        positive = command(workspace, 'bash', '-c', script, env=env)
        self.assertEqual(positive.returncode, 0, positive.stdout+positive.stderr)
        self.assertIn('0 blocking', positive.stdout)
        self.assertTrue((runner/'gates-observed-zero').is_file())

        # Candidate policy is never an authority fallback when the base bundle
        # is absent, even if the candidate carries a byte-identical judge.
        (runner/'gates-observed-zero').unlink()
        shutil.copytree(judge, target/'.harness/protected-judge/v1')
        shutil.rmtree(base/'.harness/protected-judge')
        negative = command(workspace, 'bash', '-c', script, env=env)
        self.assertNotEqual(negative.returncode, 0, negative.stdout+negative.stderr)
        self.assertIn('ADOPTION_REQUIRED', negative.stdout+negative.stderr)
        self.assertFalse((runner/'gates-observed-zero').exists())
    ...
    def test_protected_oracle_ignores_candidate_interpreter_and_imports(self):
        self.scaffold()
        for kind in ['import','interpreter']:
            with self.subTest(kind=kind):
                target=RUN/(self._testMethodName+'-'+kind);shutil.copytree(self.root,target)
                if kind=='import':
                    (target/'yaml.py').write_text('open("candidate-executed", "w").write("shadow")\nraise RuntimeError("candidate yaml module")\n')
                else:
                    tool=target/'.harness/tools/oracles-venv/bin/python3';tool.parent.mkdir(parents=True)
                    tool.write_text('#!/usr/bin/env bash\nprintf shadow > candidate-executed\nexit 0\n');tool.chmod(0o755)
                p=command(self.root,'bash',str(self.root/'scripts/verify-oracles.sh'),env={'HARNESS_TARGET_ROOT':str(target),'ORACLES_DIR':str(self.root/'.harness/oracles')})
                self.assertFalse((target/'candidate-executed').exists(),'candidate code ran inside protected oracle judge')
                self.assertEqual(p.returncode,0,p.stdout+p.stderr)
    ...
    def test_incomplete_ruleset_detail_is_not_known_absence(self):
        self.scaffold()
        self.write('Makefile','check:\n\ttest -f README.md\n')
        command(self.root,'git','remote','add','origin','https://github.com/example/repo.git')
        gh=self.write('mock/gh', '#!/usr/bin/env python3\nimport json,sys\nprint(json.dumps([{ "id":1 }] if "?" in sys.argv[2] else {"name":"required-quality-check","enforcement":"active","target":"branch","rules":[]}))\n')
        gh.chmod(0o755)
        p=command(self.root,'bash',str(KIT/'bin/harness-status.sh'),'--target',str(self.root),env={'PATH':str(gh.parent)+os.pathsep+os.environ['PATH']})
        self.assertNotEqual(p.returncode,0,p.stdout+p.stderr)
        self.assertIn('INDETERMINATE_REMOTE',p.stdout)
    ...
    def test_setup_ignores_cwd_and_pythonpath_for_pip_and_final_import(self):
        for channel in ['cwd','PYTHONPATH']:
            for phase in ['pip','yaml']:
                with self.subTest(channel=channel,phase=phase):
                    case=self.root/(channel+'-'+phase);kit=case/'kit';caller=case/'caller'
                    (kit/'scripts').mkdir(parents=True);caller.mkdir()
                    shutil.copy2(KIT/'scripts/setup-oracles.sh',kit/'scripts/setup-oracles.sh')
                    # No wheel/index/network access: pip must fail for PyYAML, or
                    # complete an empty requirements file before final import fails.
                    (kit/'requirements-harness.txt').write_text('PyYAML==6.0.3\n' if phase=='pip' else '')
                    modules=caller if channel=='cwd' else case/'modules';modules.mkdir(exist_ok=True)
                    marker=case/'project-module-ran'
                    code='from pathlib import Path\nPath('+repr(str(marker))+').write_text("executed")\n'
                    if phase=='pip':
                        (modules/'pip.py').write_text(code)
                    (modules/'yaml.py').write_text(code+'__version__ = "6.0.3"\n')
                    empty=case/'empty-wheels';empty.mkdir()
                    env={'PYTHONPATH':str(modules) if channel=='PYTHONPATH' else '',
                         'PIP_NO_INDEX':'1','PIP_CONFIG_FILE':os.devnull,
                         'PIP_FIND_LINKS':str(empty),'PIP_CACHE_DIR':str(case/'empty-cache'),
                         'PIP_DISABLE_PIP_VERSION_CHECK':'1'}
                    result=command(caller,'bash',str(kit/'scripts/setup-oracles.sh'),env=env)
                    isolated=command(caller,str(kit/'.harness/tools/oracles-venv/bin/python3'),'-I','-c','import yaml',env=env)
                    self.assertNotEqual(isolated.returncode,0,'fixture unexpectedly acquired a real parser')
                    self.assertFalse(marker.exists(),'setup executed a project-supplied '+phase+' module via '+channel)
                    self.assertNotEqual(result.returncode,0,'setup falsely reported a usable isolated parser')

    def test_status_timeout_terminates_owned_descendants_only(self):
        self.scaffold()
        for ignore_term in [False,True]:
            with self.subTest(ignore_term=ignore_term):
                suffix=str(ignore_term);late='owned-late-'+suffix;ready='owned-ready-'+suffix
                self.write('child.py','import os,signal,time\nfrom pathlib import Path\n'+
                           ('signal.signal(signal.SIGTERM,signal.SIG_IGN)\n' if ignore_term else '')+
                           'Path('+repr(ready)+').write_text(str(os.getpid()))\n'+
                           'time.sleep(1.0)\nPath('+repr(late)+').write_text("late mutation")\n')
                self.write('Makefile','check:\n\tpython3 child.py\n')
                unrelated_marker=self.root/('unrelated-'+suffix)
                unrelated=subprocess.Popen(['python3','-I','-c',
                    'import time,pathlib; time.sleep(1.2); pathlib.Path('+repr(str(unrelated_marker))+').write_text("alive")'])
                try:
                    started=time.monotonic()
                    result=command(self.root,'bash',str(KIT/'bin/harness-status.sh'),'--target',str(self.root),env={'HARNESS_STATUS_TIMEOUT':'0.2'})
                    elapsed=time.monotonic()-started
                    self.assertNotEqual(result.returncode,0,result.stdout+result.stderr)
                    self.assertIn('BLOCKED_TOOL',result.stdout)
                    self.assertTrue((self.root/ready).is_file(),'fixture child never started before timeout')
                    self.assertLess(elapsed,2.5,'status cleanup exceeded its bounded allowance')
                    self.assertEqual(unrelated.wait(timeout=3),0,'unrelated process was terminated')
                    self.assertTrue(unrelated_marker.is_file(),'unrelated process lost its work')
                    self.assertFalse((self.root/late).exists(),'owned child continued mutating after timeout')
                finally:
                    if unrelated.poll() is None: unrelated.terminate();unrelated.wait(timeout=2)

    def test_explicit_missing_delivery_source_blocks_with_default_absence_allowed(self):
        self.copy('scripts/verify-delivery-doc.sh')
        default=command(self.root,'bash','scripts/verify-delivery-doc.sh',env={'DELIVERY_DOC':'','DELIVERY_DOC_REQUIRED':'0'})
        self.assertEqual(default.returncode,0,default.stdout+default.stderr)
        for source in ['custom-release-guide.md','unreadable-directory']:
            with self.subTest(source=source):
                if source=='unreadable-directory': (self.root/source).mkdir()
                result=command(self.root,'bash','scripts/verify-delivery-doc.sh',env={'DELIVERY_DOC':source,'DELIVERY_DOC_REQUIRED':'0'})
                self.assertNotEqual(result.returncode,0,result.stdout+result.stderr)
                self.assertIn(source,result.stdout+result.stderr)
        self.assertEqual((KIT/'scripts/verify-delivery-doc.sh').read_bytes(),(KIT/'templates/full/scripts/verify-delivery-doc.sh').read_bytes())
