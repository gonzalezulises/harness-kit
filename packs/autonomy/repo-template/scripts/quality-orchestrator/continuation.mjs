...
export function continuationBoundary(runtime,auth,journal,files,acceptedRecords,internal) {
  ...
  return {
    ...
    evaluateContinuation:(wire,observed,ctx)=>safe(()=>{
      const defect=z.strictObject({gateDigest:hash,path:id}).parse(observed);
      const value=verify(wire,ctx,defect),grantDigest=digestData(value.wire.grant),key='grant:'+grantDigest;
