...
"$PARSER" -I -c 'import sys; assert sys.version_info >= (3,8); import yaml; assert yaml.__version__ == "6.0.3"' >/dev/null 2>&1 || {
  echo 'TOOL_FAILURE: Python3.8+ and PyYAML==6.0.3 required; run bash scripts/setup-oracles.sh' >&2; exit 3;
}
if [[ "$MODE" == full ]]; then
